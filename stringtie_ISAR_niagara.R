...
aSwitchList <- importRdata(isoformCountMatrix = stringtie$counts, isoformRepExpression = stringtie$abundance, designMatrix = myDesign, isoformExonAnnoation = "all_merged_stringtie.gtf")
...
write.table(aSwitchList$isoformFeatures, "stringtie_isoform_features.tsv", col.names = T, row.names = F, sep = "\t", quote = F)
...
filter_by_conditions <- function (full_switch_list, cond1, cond2){
  ...
  write.table(sub_switch_list, paste("dAS_analysis/dAS_", cond1, cond2, "_stringtie.txt", sep = ""), col.names = T, row.names = F, sep = "\t", quote = F)
}
